...
async def get_ws_endpoints(device_id: str, user_id: str, proxy_url: str):
    ...
    async with ClientSession(connector=connector) as session:
        try:
            async with session.post(url, json=data, headers=HEADERS, proxy=proxy_url if proxy_url else None) as response:
...
class ProxyManager:
    ...
    async def start(self, target_connections: int = 500) -> None:
        ...
        selected = random.sample(list(available_proxies), min(len(available_proxies), target_connections))
        self.active_proxies = set(selected)
        proxy_device_pairs = list(zip(selected, self.device_ids[:len(selected)]))
        tasks = {asyncio.create_task(self._run_client(proxy, device_id)): (proxy, device_id) for proxy, device_id in proxy_device_pairs}

        while True:
            await asyncio.sleep(5)
            done, _ = await asyncio.wait(tasks.keys(), timeout=0, return_when=asyncio.FIRST_COMPLETED)
            ...
            while len(tasks) < target_connections:
                await self.load_proxies()
                remaining = {p for p in self.all_proxies if p not in self.active_proxies and (p not in BANNED_PROXIES or time.time() >= BANNED_PROXIES[p])}
                if not remaining:
                    logger.warning("⚠️ No proxies left for refill.")
                    break
...
async def user_input() -> dict:
    user_ids_input = input(f"🔑 Enter your USER IDs (comma separated): ")
...
async def device_input(existing_count: int) -> list:
    if existing_count > 0:
        use_existing = input(f"🔑 You have {existing_count} devices already configured. Do you want to use them? (yes/no): ").strip().lower()
        if use_existing == "yes":
            return await load_device_ids()

    num_devices_input = input(f"🔑 Enter the number of devices you want to create: ")
...
async def main() -> None:
    ...
    for user_id in user_ids:
        proxy_manager = ProxyManager(device_ids, user_id)
        await proxy_manager.start(target_connections=500)
